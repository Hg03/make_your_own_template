...
I want my working directory to be {workdir} and I want to copy {copy} from my project files.
...
We don't work with requirements.text, we use pyproject.toml for dependency management. Keep the CMD instruction to be empty list.
"""

def render_dockerfile(base_image, run_options, workdir, copy, expose_port, extras):
    return docker_prompt_template.format(base_image=base_image, run_options=", ".join(run_options) if run_options else "no additional packages",
                                         workdir=workdir, copy=copy, expose_port=expose_port, extras=extras if extras else "no extra commands")